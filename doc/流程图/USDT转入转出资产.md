...
graph TB
    ...
    hasNew -- 有 --> checkConfirmed{区块链交易已到账?}
    hasNew -- 无 --> stop(结束)
...
```mermaid
graph TB
    start(开始) --> checkBalance{用户USDT余额充足?}
    ...
    checkPhone -- 是 --> transfer[区块链转出USDT]
    checkPhone -- 否 --> stop(结束)
    checkAddress -- 是 --> transfer
    checkAddress -- 否 --> checkLimit{达到转出数量审核门槛?}
    checkLimit -- 是 --> examine{等待管理员审核}
    checkLimit -- 否 --> transfer
    examine -- 通过 --> transfer
    examine -- 拒绝 --> stop
    transfer --> stop
```